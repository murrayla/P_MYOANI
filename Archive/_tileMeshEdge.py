...
def msh_(mesh_size):
    ...
    # # ∆ Set mesh size
    # for p in gmsh.model.getEntities(dim=1):
    #     gmsh.model.mesh.setSize([p], mesh_size)

    # ∆ Create a distance field from x-faces
    gmsh.model.mesh.field.add("Distance", 1)
    gmsh.model.mesh.field.setNumbers(1, "FacesList", x_face)

    gmsh.model.mesh.field.add("Threshold", 2)
    gmsh.model.mesh.field.setNumber(2, "InField", 1)
    gmsh.model.mesh.field.setNumber(2, "SizeMin", mesh_size)
    gmsh.model.mesh.field.setNumber(2, "SizeMax", mesh_size*3)
    gmsh.model.mesh.field.setNumber(2, "DistMin", 1000)
    gmsh.model.mesh.field.setNumber(2, "DistMax", 2000)

    # ∆ Combine both fields using Min
    gmsh.model.mesh.field.add("Min", 9)
    gmsh.model.mesh.field.setNumbers(9, "FieldsList", [2])
