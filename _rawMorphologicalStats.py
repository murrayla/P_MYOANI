...
def validate_data(data, norm_df, iso_df):
    ...
    for i, r in reg_df.iterrows():
        ...
        # ∆ Overlay the complete dataset histogram
        sns.histplot(norm_df['Azi_[RAD]'], ax=ax, stat='density', color='orange', alpha=0.3, bins=20, label='Complete Dataset')

        # ∆ Collect Azi_[RAD] values within the current region
        ...
        for _, row in norm_df.iterrows():
            ...
            ang_reg_df = pd.concat([ang_reg_df, pd.DataFrame({'Azi_[RAD]': [row["Azi_[RAD]"]]})], ignore_index=True)

        # ∆ Plot the histogram as proportion for the current region
        if not ang_reg_df.empty:
            sns.histplot(ang_reg_df['Azi_[RAD]'], ax=ax, stat='density', edgecolor='black', alpha=0.7, bins=20)
        else:
            ax.text(0.5, 0.5, 'No data in this region', ha='center', va='center', transform=ax.transAxes)

        # ∆ Add a legend (only need to do this once, but placing it here ensures it's associated with the axes)
        ax.legend()

    plt.tight_layout()
    plt.savefig(f"_png/TILES_PROP_AZI_OVERLAY_HIST.png", bbox_inches='tight', pad_inches=0.2, dpi=1000)
    plt.close()
