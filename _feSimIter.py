"""
    ...
    File Name: _feSimIter.py
       finite element simulation with FENICS and iterations
"""
...
def fx_(t, file, m_ref, gcc, sim, dpm):
    ...
    # ∆ Iterate solutions
    for jj, kk in enumerate([0, 5, 10, 15, 20, 15, 10, 5, 0]):
        ...
        # ∆ Save CSV
        df.to_csv(f"_csv/{t}_{dpm}_{kk}.csv")  
...
if __name__ == '__main__':
    ...
    # ∆ Indicate test cases
    tests = ["test"] + [x for x in [str(y) for y in range(0, 18, 1)]]
    # tests = ["0"]
    m_ref = 1
    dpm = 20
    sim = "N" + str(dpm)
