...
def fx_(t, file, gcc, r):
    ...
    Args:
        ...
        r (int): Refinement parameter (appears unused in current context but kept).
    ...
    for ii, kk in enumerate([0, 5, 10, 15, 20]):
        ...
        print(len(disp_arr)) # Debug print, consider removing or making conditional

        # Store data in pandas DataFrames
        coords_snapshot = np.array(x_n.function_space.tabulate_dof_coordinates()[:]) # Re-tabulate for current state
        df = pd.DataFrame(
            data={
                "X": coords_snapshot[:, 0], "Y": coords_snapshot[:, 1], "Z": coords_snapshot[:, 2],
        ...
        print(len(coords_snapshot[:, 0]), len(r_disp[:, 0])) # Debug print, consider removing or making conditional
        ...
        # Calculate mean stress at specific points
        sigs = []
...
def main(tests, m_ref):
    ...
    Args:
        ...
        m_ref (int): Mesh refinement parameter.
    """
...
if __name__ == '__main__':
    ...
    r = 400 # Refinement parameter (passed to fx_, but its usage is unclear)
    main(refs, r)
